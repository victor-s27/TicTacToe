...
class MainActivity : AppCompatActivity() {
    ...
    private fun onClick(btn: Button) {
        if(btn.text.isNotBlank()) return
        btn.text = if(turnOfPlayer1) "X" else "O"
        moveCount++
    ...
    * */
    ...
    private fun enterNameOfPlayer2() {
        val builder: android.app.AlertDialog.Builder = android.app.AlertDialog.Builder(this)
        builder.setTitle("Enter the name for second (noughts) player!")
        val input = EditText(this)
        input.hint = "Enter Name"
        input.inputType = InputType.TYPE_CLASS_TEXT
        builder.setView(input)
        builder.setPositiveButton("OK") { _, _ ->
            nameOfPlayer2 = input.text.toString()
            namePlayer2.text = "$nameOfPlayer2: $pointsOfPlayer2"
        }
        builder.setNegativeButton("DEFAULT") { _, _ -> }.show()
